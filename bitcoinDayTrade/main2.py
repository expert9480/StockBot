...
COOLDOWN_SECONDS = 60 * 1     # 30 minutes between flips (except emergency exit if you add one)

# Allocation behavior
# - When bullish: hold BTC with as much cash as possible (up to STARTING_MONEY_USD notionals)
# - When bearish & market open: hold BITI with as much cash as possible
# - When bearish & market closed: go to cash (sell BTC, cannot buy BITI)
...
def market_is_open() -> bool:
    # Uses Alpaca clock (equities market)
    c = alpaca.get_clock()
...
def get_last_n_btc_closes_5m(n: int) -> list[float]:
    ...
    closes = [float(x) for x in bars["close"].iloc[-n:].tolist()]
    return closes
...
def place_equity_buy_notional(symbol: str, usd: float) -> str:
    """
    For equities, not all accounts support notional for all symbols; if it errors,
    switch to share-based sizing.
    """
    o = alpaca.submit_order(
        symbol=symbol,
        notional=round(usd, 2),
...
def compute_signal_from_last3(closes: list[float]) -> Signal:
    # closes length should be 4 so we can see 3 deltas
    # e.g. [c0, c1, c2, c3]
    c0, c1, c2, c3 = closes
    d1 = c1 - c0
    d2 = c2 - c1
...
def rotate_to_cash() -> bool:
    """
    Sell BTC and sell BITI (if any). Return True if any order placed.
    """
    did = False
...
def rotate_to_btc(price: float) -> bool:
    """
    Ensure BITI is sold, then buy BTC with available cash up to STARTING_MONEY_USD.
    """
    did = False
    ...
    cash = safe_get_cash()
    # cap buy to STARTING_MONEY_USD (simple)
...
def rotate_to_biti() -> bool:
    """
    Ensure BTC is sold, then buy BITI with cash up to STARTING_MONEY_USD.
    Only call this when market is open.
    """
    did = False
...
def main() -> None:
    ...
    while True:
        try:
            # Don’t place new orders if we already have open ones for these symbols
            ...
            if open_orders:
                ...
                # Skip if same bar already processed (restart-safe)
                if last_bar_time is not None and bar_time <= last_bar_time:
                    ...
                    log_line(
                        f"BTC 5m close={price:.2f} | signal={sig.direction} move={sig.move_pct*100:.3f}% | market_open={is_open}"
                    )
                    ...
                    if last_trade_time is not None:
                        ...
                        if since < COOLDOWN_SECONDS:
                            ...
                            # still allow reporting below
                        else:
                            # Only act on strong-enough signals
                            if sig.direction in ("UP", "DOWN") and sig.move_pct >= MIN_MOVE_PCT_3BARS:
                                ...
                                if sig.direction == "UP":
                                    ...
                                    if is_open:
                                        ...
                                        # Market closed: go to cash rather than trying BITI
                    ...
                    # else:
                    #     # First run: don’t immediately trade; wait until cooldown baseline is set
                    #     log_line("INFO: No last_trade_time in state yet; setting now (prevents immediate flip).")
                    #     last_trade_time = now_utc()
                    #     set_dt(state, "last_trade_time", last_trade_time)
                    #     save_state(state)
                    else:
                        # First run: if we have NO holdings, allow an initial allocation immediately.
                        if not has_any_holdings():
                            log_line("INFO: No holdings detected (BTC=0, BITI=0). Performing initial allocation if signal allows.")

                            # Only allocate if we have a clean signal + enough move (same rules)
                            if sig.direction in ("UP", "DOWN") and sig.move_pct >= MIN_MOVE_PCT_3BARS:
                                did_trade = False
                                if sig.direction == "UP":
                                    did_trade = rotate_to_btc(price)
                                ...
                                if did_trade:
                                    ...
                                    # If we didn't trade, still set last_trade_time so we don't loop-wig out
                                    last_trade_time = now_utc()
                                    set_dt(state, "last_trade_time", last_trade_time)
                                    save_state(state)
                            else:
                                log_line("INFO: Initial allocation skipped (no clean signal / min move not met). Staying in CASH.")
                                last_trade_time = now_utc()
                                set_dt(state, "last_trade_time", last_trade_time)
                                save_state(state)

                        else:
                            # If holdings already exist, just set trade time baseline and proceed normally.
                            log_line("INFO: Holdings exist; setting last_trade_time baseline.")
                            last_trade_time = now_utc()
                            set_dt(state, "last_trade_time", last_trade_time)
                            save_state(state)
            ...
            if is_open_now:
                if last_open_report_time is None:
                    # First open-market report anchor
                ...
                # Market closed: do not send scheduled reports.
                # Reset open report anchor so next open starts a fresh 2-hour window.
